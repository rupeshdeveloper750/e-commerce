    ...
    <div class="overflow-hidden rounded-3xl border border-slate-800 bg-[#111827] shadow-xl">
        ...
        <div class="flex items-center justify-between border-b border-slate-800 px-6 py-5">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-400">View status, check inventory alerts, and configure item models.</p>
            </div>
            <div class="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-slate-300">
                Total : <span class="font-semibold text-white">{{ $products->total() }}</span>
            </div>
        </div>
            ...
            <table class="min-w-full">
                <thead class="border-b border-slate-800 bg-slate-900">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Category / Brand</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Price</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Stock Status</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Visibility</th>
                        <th class="px-6 py-4 text-center text-sm font-semibold text-slate-300">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800">
                    @forelse ($products as $product)
                        <tr class="hover:bg-slate-900/40 transition {{ $product->trashed() ? 'bg-red-950/5' : '' }}">
                            <td class="px-6 py-4">
                                <input
                                    type="checkbox"
                                    :value="{{ $product->id }}"
                            ...
                            <td class="px-6 py-4">
                                @if ($product->featuredImage)
                                    <img src="{{ asset('storage/' . $product->featuredImage->image_path) }}" class="h-10 w-10 rounded-lg object-cover border border-slate-800" alt="{{ $product->name }}">
                                @elseif ($product->images->first())
                                    <img src="{{ asset('storage/' . $product->images->first()->image_path) }}" class="h-10 w-10 rounded-lg object-cover border border-slate-800" alt="{{ $product->name }}">
                                @else
                            ...
                            <td class="px-6 py-4 text-sm font-semibold text-white">
                                ...
                                <div class="text-xs text-slate-500 font-normal mt-0.5">SKU: {{ $product->sku }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-400">
                                <div>{{ $product->category ? $product->category->name : '-' }}</div>
                                <div class="text-xs text-slate-500 mt-0.5">{{ $product->brand ? $product->brand->name : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm font-semibold text-white">
                                @if ($product->sale_price)
                                    <span class="text-amber-500">₹{{ number_format($product->sale_price, 2) }}</span>
                                    <span class="text-xs text-slate-500 line-through ml-1.5">₹{{ number_format($product->price, 2) }}</span>
                                @else
                                    ₹{{ number_format($product->price, 2) }}
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-400">
                                <div class="flex items-center gap-2">
                                    <span class="font-medium text-slate-200">{{ $product->quantity }} units</span>
                                    @if ($product->quantity <= 0)
                                        <span class="inline-flex items-center rounded bg-red-400/10 px-1.5 py-0.5 text-2xs font-medium text-red-400 ring-1 ring-inset ring-red-400/20">Out of Stock</span>
                                    @elseif ($product->quantity <= 5)
                                        <span class="inline-flex items-center rounded bg-amber-400/10 px-1.5 py-0.5 text-2xs font-medium text-amber-400 ring-1 ring-inset ring-amber-400/20">Low Stock</span>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $product->status ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-800' }}">
                                    {{ $product->status ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
