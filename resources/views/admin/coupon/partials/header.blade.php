<div x-data="{ showImport: false }" class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
    ...
    <div>
        ...
        <nav class="mb-3 flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-400">/</span>
            <span class="font-medium text-[#B88A44]">Coupons</span>
        </nav>
        ...
        <p class="mt-2 text-sm text-gray-500 dark:text-slate-400">
            Create discount promo codes, apply constraints and control user activations.
        </p>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        {{-- Export CSV --}}
        <a
            href="{{ route('admin.coupons.export') }}"
            class="inline-flex h-12 items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-5 text-sm font-semibold text-slate-300 transition hover:bg-slate-700"
        >
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    >
        <div class="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
                <div>
                    <h3 class="text-lg font-bold text-white">Import Coupons from CSV</h3>
                    <p class="mt-1 text-sm text-slate-400">Upload a CSV file containing coupons.</p>
                </div>

                <form action="{{ route('admin.coupons.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
