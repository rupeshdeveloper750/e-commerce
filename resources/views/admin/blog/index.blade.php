...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <nav class="mb-3 flex items-center gap-2 text-sm">
                ...
                <span class="text-gray-400">/</span>
                <span class="font-medium text-[#B88A44]">Blogs</span>
            </nav>
            <h1 class="text-3xl font-bold tracking-tight text-white">Blog Publications</h1>
            <p class="mt-2 text-sm text-slate-400">Compose and edit marketing articles, product updates and announcements.</p>
        </div>
        <div>
            <a href="{{ route('admin.blogs.create') }}" class="inline-flex h-12 items-center gap-2 rounded-xl bg-[#B88A44] px-5 text-sm font-semibold text-white shadow-lg transition hover:bg-[#a67936]">
                + Write Article
            </a>
        </div>
    </div>
        ...
        <form action="{{ route('admin.blogs.index') }}" method="GET">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-12">
                <div class="lg:col-span-6">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search title, content..." class="h-14 w-full rounded-2xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
                </div>
            ...
            <div class="mt-6 flex justify-end gap-3 border-t border-slate-800 pt-4">
                <a href="{{ route('admin.blogs.index') }}" class="inline-flex h-10 items-center justify-center rounded-xl border border-slate-700 bg-slate-800 px-5 text-sm font-semibold text-slate-300 hover:text-white transition">Reset</a>
    ...
    <div x-show="selectedIds.length > 0" class="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-4 rounded-2xl border border-slate-800 bg-slate-900/90 px-6 py-4 shadow-2xl backdrop-blur-md" style="display: none;">
        ...
        <form action="{{ route('admin.blogs.bulk-action') }}" method="POST" class="flex items-center gap-3">
            @csrf
            ...
            <select name="action" class="rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs text-slate-200 focus:outline-none">
                <option value="block">Make Draft</option>
            ...
            <table class="min-w-full">
                <thead class="border-b border-slate-800 bg-slate-900">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Cover Image</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Article Title</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Status</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Published At</th>
                        <th class="px-6 py-4 text-center text-sm font-semibold text-slate-300">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-800">
                    @forelse($blogs as $blog)
                        <tr class="hover:bg-slate-900/40 transition {{ $blog->trashed() ? 'bg-red-950/5' : '' }}">
                            <td class="px-6 py-4">
                                <input type="checkbox" :value="{{ $blog->id }}" :checked="selectedIds.includes({{ $blog->id }})" @click="toggleItem({{ $blog->id }})" class="item-checkbox h-4 w-4 rounded border-slate-700 bg-slate-800 text-[#B88A44] focus:ring-[#B88A44]">
                            </td>
                            <td class="px-6 py-4">
                                @if($blog->image)
                                    <img src="{{ asset('storage/' . $blog->image) }}" class="h-10 w-16 rounded-lg object-cover border border-slate-800" alt="{{ $blog->title }}">
                                @else
                                    <div class="flex h-10 w-16 items-center justify-center rounded-lg bg-slate-800 text-slate-500 font-bold border border-slate-700 text-xs">
                                        No Image
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm font-semibold text-white">
                                ...
                                <div class="text-xs text-slate-505 font-normal mt-0.5">{{ $blog->slug }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $blog->status ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-800' }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2.5">
                                    @if(!$blog->trashed())
                                        <a href="{{ route('admin.blogs.edit', $blog->id) }}" class="rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:text-white transition">Edit</a>
                                        <form action="{{ route('admin.blogs.destroy', $blog->id) }}" method="POST" onsubmit="return confirm('Soft delete this blog article?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        ...
                                        <form action="{{ route('admin.blogs.force-delete', $blog->id) }}" method="POST" onsubmit="return confirm('WARNING: Permanently delete this blog article?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-sm text-slate-500">No blog publications found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
