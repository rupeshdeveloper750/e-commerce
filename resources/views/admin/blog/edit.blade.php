...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div>
        <nav class="mb-3 flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-400">/</span>
            <span class="font-medium text-[#B88A44]">Edit Article</span>
        </nav>
        <h1 class="text-3xl font-bold tracking-tight text-white">Edit Blog Article: {{ $blog->title }}</h1>
    </div>
    ...
    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="rounded-3xl border border-slate-800 bg-[#111827] p-6 space-y-5">
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $blog->title) }}" required class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug', $blog->slug) }}" class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
                ...
                <div>
                    ...
                    <select name="status" class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
                        <option value="1" {{ old('status', $blog->status) == 1 ? 'selected' : '' }}>Publish Immediately</option>
                        <option value="0" {{ old('status', $blog->status) == 0 ? 'selected' : '' }}>Save as Draft</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="content" rows="12" class="w-full rounded-xl border border-slate-700 bg-slate-800 p-4 text-white focus:border-[#B88A44] focus:outline-none">{{ old('content', $blog->content) }}</textarea>
            ...
            <div class="border-t border-slate-800 pt-5">
                ...
                @if($blog->image)
                    <div class="mt-4">
                        <p class="text-xs text-slate-500 mb-2">Current Cover Image</p>
                        <img src="{{ asset('storage/' . $blog->image) }}" class="h-24 w-40 rounded-xl object-cover border border-slate-800">
                    </div>
                @endif
            </div>
            ...
            <div class="border-t border-slate-800 pt-5 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title', $blog->meta_title) }}" class="h-10 w-full rounded-lg border border-slate-700 bg-slate-800 px-3 text-sm text-white focus:border-[#B88A44] focus:outline-none">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-400 mb-1">Meta Description</label>
                    <textarea name="meta_description" rows="3" class="w-full rounded-lg border border-slate-700 bg-slate-800 p-3 text-sm text-white focus:border-[#B88A44] focus:outline-none">{{ old('meta_description', $blog->meta_description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <button type="submit" class="inline-flex h-12 items-center justify-center rounded-xl bg-[#B88A44] px-8 text-sm font-semibold text-white transition hover:bg-[#a67936]">Update Article</button>
        </div>
    </form>
</div>
@endsection
