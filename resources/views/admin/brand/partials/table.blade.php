<div x-data="{
    ...
    >
        ...
        <form action="{{ route('admin.brands.bulk-action') }}" method="POST" class="flex items-center gap-3">
            @csrf
    ...
    <div class="overflow-hidden rounded-3xl border border-slate-800 bg-[#111827] shadow-xl">
        ...
        <div class="flex items-center justify-between border-b border-slate-800 px-6 py-5">
            <div>
                <h3 class="text-lg font-semibold text-white">Brands List</h3>
                <p class="mt-1 text-sm text-slate-400">View and update brand identities, active statuses and featured markers.</p>
            </div>
            <div class="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-slate-300">
                Total : <span class="font-semibold text-white">{{ $brands->total() }}</span>
            </div>
        </div>
            ...
            <table class="min-w-full">
                <thead class="border-b border-slate-800 bg-slate-900">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Logo</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Brand</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Featured</th>
                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse ($brands as $brand)
                        <tr class="hover:bg-slate-900/40 transition {{ $brand->trashed() ? 'bg-red-950/5' : '' }}">
                            <td class="px-6 py-4">
                                <input
                                    type="checkbox"
                                    :value="{{ $brand->id }}"
                                    :checked="selectedIds.includes({{ $brand->id }})"
                                    @click="toggleItem({{ $brand->id }})"
                                    class="item-checkbox h-4 w-4 rounded border-slate-700 bg-slate-800 text-[#B88A44] focus:ring-[#B88A44]"
                                >
                            </td>
                            <td class="px-6 py-4">
                                @if ($brand->image)
                                    <img src="{{ asset('storage/' . $brand->image) }}" class="h-10 w-10 rounded-lg object-cover border border-slate-800" alt="{{ $brand->name }}">
                                @else
                            ...
                            <td class="px-6 py-4 text-sm font-semibold text-white">
                                ...
                                <div class="text-xs text-slate-500 font-normal mt-0.5">{{ $brand->slug }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $brand->is_featured ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-slate-800 text-slate-500 border border-slate-800' }}">
                                    {{ $brand->is_featured ? 'Yes' : 'No' }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $brand->status ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-800' }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2.5">
                                    @if (!$brand->trashed())
                                        ...
                                        <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to soft delete this brand?');">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                type="submit"
                                    ...
                                    @else
                                        ...
                                        <form action="{{ route('admin.brands.force-delete', $brand->id) }}" method="POST" onsubmit="return confirm('WARNING: Permanently delete this brand? This will orphan products linked to it.');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                type="submit"
