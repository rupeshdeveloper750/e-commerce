...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        <nav class="mb-3 flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-400">/</span>
            <span class="font-medium text-[#B88A44]">Edit Customer</span>
        </nav>
        <h1 class="text-3xl font-bold tracking-tight text-white">Edit Customer: {{ $customer->name }}</h1>
    </div>
    ...
    <form action="{{ route('admin.customers.update', $customer->id) }}" method="POST" class="space-y-6">
        ...
        <div class="rounded-3xl border border-slate-800 bg-[#111827] p-6 space-y-5">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $customer->name) }}" required class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
                @error('name') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Email Address</label>
                <input type="email" name="email" value="{{ old('email', $customer->email) }}" required class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
            ...
            <div>
                ...
                <select name="status" class="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 text-white focus:border-[#B88A44] focus:outline-none">
                    ...
                    <option value="0" {{ old('status', $customer->status) == 0 ? 'selected' : '' }}>Blocked</option>
                </select>
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <button type="submit" class="inline-flex h-12 items-center justify-center rounded-xl bg-[#B88A44] px-8 text-sm font-semibold text-white transition hover:bg-[#a67936]">Update Customer</button>
        </div>
    </form>
</div>
@endsection
